...
try:
    sys.path.append(glob.glob('/home/michelle/carla_sim_8/PythonAPI/carla/dist/carla-*%d.%d-%s.egg' % (
...
try:
    from example_threaded import run
except ImportError:
    raise RuntimeError('cannot import threaded example')
...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        self.example_thread = threading.Thread(target=run, args=(args, self.client))
...
def main():
    ...
    parser.add_argument('--res', metavar='WIDTHxHEIGHT', default='640x480', help='window resolution (default: 640x480)')
    parser.add_argument("--output_type", default="no_recording", help="Type of output for images, can be one of the following: [pandas, numpy, json, folder].")
    parser.add_argument("--output_path", default="./", help="Path to save the data. (Current directory as default)")
    parser.add_argument("--preview", default=False, action='store_true', help="Visualize the camera sensors")
    parser.add_argument('--fps', default='50.0', type=float, help='Frame per second of the simulation (50fps by default)')
    parser.add_argument("--autopilot", default=False, action='store_true', help="Set autopilot")
    parser.add_argument("--depth", default="cc.LogarithmicDepth", help="Depth type of the Depth image: cc.Raw, cc.Depth or cc.LogarithmicDepth (cc.LogarithmicDepth as default)")
    parser.add_argument("--number_samples", default="-1", type=int, help="Number of desired samples (infinite loop by default)")
    arguments = parser.parse_args()

    assert arguments.output_type in ["pandas", "numpy", "json", "folder", "no_recording"]
    assert os.path.isdir(arguments.output_path), "%s should be valid dir." % arguments.output_path
