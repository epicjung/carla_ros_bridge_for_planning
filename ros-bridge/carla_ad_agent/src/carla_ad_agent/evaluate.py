...
class Evaluator():
    def __init__(self, role_name, goal):
        """
        Constructor
        """
    ...
    def path_handler(self, path):
        ...
        if len(self._global_plan.poses) > 1:
            ...
            rospy.loginfo("New plan with {} waypoints (real: {} ) received.".format(len(self._global_plan.poses), self._route_length))
        ...
        while not rospy.is_shutdown():
            if self._odom_initialized:
                    ...
                    for actor in actor_list:
                        if "role_name" in actor.attributes:
                            if actor.attributes["role_name"] == 'ego_vehicle':
                                ego_vehicle = actor
                    if ego_vehicle:
                        self._collision_sensor = world.spawn_actor(blueprint_library.find('sensor.other.collision'), carla.Transform(), attach_to=ego_vehicle)
                        self._lane_invasion_sensor = world.spawn_actor(blueprint_library.find('sensor.other.lane_invasion'), carla.Transform(), attach_to=ego_vehicle)
                ...
                if self._route_assigned:
                    ...
                    min_idx = -1
                    for i in range(len(self._global_plan.poses)):
                        dist = self.point_distance(self._current_waypoint.pose.position, self._global_plan.poses[i].pose.position)
                        if dist < min_dist:
                            min_dist = dist
                            min_idx = i
                    if min_idx >= 0:
                ...
                if distance_to_goal <= 4.0 and self._current_speed == 0.0:
                    if not self._on_goal_start_timer:
                        ...
                        time_diff = time.time() - self._on_goal_start_timer
...
def main():
    ...
    goal = rospy.get_param("~goal", '-330.16,415.28,0,0,0,180')
    evaluator = Evaluator(role_name, goal)
