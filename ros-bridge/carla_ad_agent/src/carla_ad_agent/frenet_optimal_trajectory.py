...
MAX_CURVATURE = 20.0  # maximum curvature [1/m]
    ...
    for fp in fplist:
        ...
        for i in range(len(fp.x) - 1):
            ...
            if diff == 0.0:
                is_valid = False
                break
            fp.yaw.append(yaw_angle)
            fp.ds.append(diff)

        # check path validity 
        if not is_valid or len(fp.x) == 1:
            continue
...
def check_paths(fplist, ob):
    ...
    for i in range(len(fplist)):
        ...
        #     print("max accel check: {}").format(fplist[i].s_dd)
        #     continue
        elif any([abs(c) > MAX_CURVATURE for c in fplist[i].c]):  # Max curvature check
            print("max curvature check")
            continue
        elif ob.size > 0 and not check_collision(fplist[i], ob):
...
def main():
    ...
    c_d = -5.0  # current lateral position [m]
